...
namespace conct
{
	...
	public:
		...
		Command*					setProperty( const RemoteInstance& instance, const char* pName, const Value& value );

		ValueCommand*				callFunction( const RemoteInstance& instance, const char* pName, const ConstArrayView< Value >& arguments );
	...
	private:
		...
		TCommand*					beginCommand( const DeviceAddress& deviceAddress, const ConstArrayView< byte >& payload, MessageType messageType );
	};
}
